...
로우폴리 3인칭 케이블 스윙 타임어택. 시작 화면 오른쪽 목록에 6개 맵이 코스 모양·난이도·내 최고 기록과 함께 모두 보인다. 누르면 배경이 그 맵으로 바뀌고, 선택된 맵을 한 번 더 누르면 바로 출발. 키보드는 1~6 바로 선택, ↑↓/←→ 이동, Enter 출발. 모바일에서는 가로 스크롤 칩으로 선택. 결과 화면의 "다른 맵 고르기"로 곧장 목록으로 돌아간다.

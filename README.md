...
`npm test`로 보정 없이 훅 조종만으로 모든 맵을 완주할 수 있는지, 훅 없이 W만 누르면 코스를 따라 휘지 않고 직진하는지, 곡선 트랙 투영·벽 충돌, 추락 복귀, 체크포인트 순서, 타이머, 기록 데이터, Supabase 랭킹 SQL(실제 마이그레이션을 PGlite로 실행) 검증.
...
- 코스 양옆 벽에 닿으면 미끄러지는 보정 없이 바로 충돌로 처리되어, 게이트를 놓쳤을 때처럼 마지막 체크포인트로 돌아가며 3초가 추가된다. 벽 6m 안으로 다가가면 화면 옆에 "벽 주의"가 뜨고, 2.5m 안에서는 빨갛게 깜빡인다.
- 훅은 내 진행 방향 기준 앞쪽·왼쪽/오른쪽의 연결점에 걸린다. 곡선에서는 가만히 있으면 바깥 벽으로 날아가므로, 돌고 싶은 쪽 훅으로 직접 당겨 방향을 틀어야 한다. 연결점은 벽 바깥 건물에 붙어 있어 한쪽 훅만 잡고 끝까지 흔들리면 그쪽 벽에 부딪힌다.
- R 처음부터 다시, Escape 일시정지. 다른 창으로 전환하면 자동 일시정지.
- 모바일: 왼쪽 훅, 오른쪽 훅, W/S 버튼. 여러 손가락 동시 입력 지원.
- 게이트를 순서대로 통과하면 완주. 추락하거나 벽에 닿거나 게이트를 놓치면 마지막 체크포인트로 돌아가며 3초 추가.

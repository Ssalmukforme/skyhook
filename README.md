...
`npm test`로 모든 맵을 벽 충돌 없이 완주할 수 있는지(벽 반대쪽 훅으로 조종하는 자동 주행), 곡선 트랙 투영·벽 충돌, 추락 복귀, 체크포인트 순서, 타이머, 기록 데이터, Supabase 랭킹 SQL(실제 마이그레이션을 PGlite로 실행) 검증.
...
- 곡선에서는 관성 때문에 바깥쪽 벽으로 쏠린다. 벽 반대쪽 훅만 잡아 당기면 코스를 따라 돌 수 있다. 양쪽 훅을 함께 잡으면 옆 흔들림이 빨리 가라앉아 중앙을 유지하기 쉽다.
